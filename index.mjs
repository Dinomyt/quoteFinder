...
app.get("/searchByKeyword", async (req, res) => {
  try {
    ...
    const sql = `SELECT quoteid, quote, authorid, firstname, lastname FROM q_quotes NATURAL JOIN q_authors WHERE quote ILIKE $1`;
    const { rows } = await pool.query(sql, [`%${keyword}%`]);
    res.render("results", { quotes: rows });
  } catch (err) {
    console.error(err);
    res.status(500).send("Error during keyword search.");
  }
});
...
app.get("/searchByAuthor", async (req, res) => {
  try {
    ...
    const sql = `SELECT quoteid, quote, authorid, firstname, lastname FROM q_quotes NATURAL JOIN q_authors WHERE authorid = $1`;
    const { rows } = await pool.query(sql, [authorId]);
    res.render("results", { quotes: rows });
  } catch (err) {
    console.error(err);
    res.status(500).send("Error during author search.");
  }
});
...
app.get("/searchByCategory", async (req, res) => {
  try {
    ...
    const sql = `SELECT quoteid, quote, authorid, firstname, lastname FROM q_quotes NATURAL JOIN q_authors WHERE category = $1`;
    const { rows } = await pool.query(sql, [category]);
    res.render("results", { quotes: rows });
  } catch (err) {
    console.error(err);
    res.status(500).send("Error during category search.");
  }
});
...
app.get("/searchByLikes", async (req, res) => {
  try {
    ...
    const sql = `SELECT quoteid, quote, authorid, firstname, lastname FROM q_quotes NATURAL JOIN q_authors WHERE likes BETWEEN $1 AND $2`;
    const { rows } = await pool.query(sql, [minLikes, maxLikes]);
    res.render("results", { quotes: rows });
  } catch (err) {
    console.error(err);
    res.status(500).send("Error during likes search.");
  }
});
